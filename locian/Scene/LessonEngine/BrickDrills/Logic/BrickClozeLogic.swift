...
class BrickClozeLogic: ObservableObject {
    ...
    private static let fullIntroVoices = [
        "Fill in the blank for \"%@\" in %@.",
        "What's missing for \"%@\" in %@?",
        "Complete the %@ phrase for \"%@\".",
        "Which %@ word means \"%@\"?",
        "Finish the sentence for \"%@\" in %@."
    ]
    ...
    static func playIntro(drill: DrillState, engine: LessonEngine, mode: DrillMode) {
        if let override = drill.overrideVoiceInstructions {
            print("🎙️ [BrickCloze] Using Voice Override: '\(override)'")
    ...
    private func playFeedback(isCorrect: Bool) {
        ...
        let template = isCorrect ? 
            (BrickClozeLogic.correctVoices.randomElement() ?? "Correct! \"%@\" in %@ is \"%@\"") :
            (BrickClozeLogic.wrongVoices.randomElement() ?? "Actually, \"%@\" in %@ is \"%@\"")
    ...
    static func view(for state: DrillState, mode: DrillMode, engine: LessonEngine, patternIntroLogic: PatternIntroLogic? = nil, practiceLogic: PatternPracticeLogic? = nil, ghostLogic: GhostModeLogic? = nil, onComplete: ((Bool) -> Void)? = nil) -> some View {
        if let introLogic = patternIntroLogic {
            ...
            BrickClozeView(state: state, engine: engine, patternIntroLogic: patternIntroLogic, practiceLogic: practiceLogic, ghostLogic: ghostLogic, onComplete: onComplete)
                .onAppear {
                    BrickClozeLogic.playIntro(drill: state, engine: engine, mode: mode)
                }
        }
