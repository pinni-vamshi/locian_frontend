...
class MCQOptionGenerator {
    ...
    // MARK: - Two-Stage MCQ Distractors

    /// Stage 1 distractors: 4 dictionary (base) forms.
    /// Correct = `targetBase`. Distractors are other base forms drawn from the
    /// current lesson's bricks (same `baseKind` preferred), then any base, then
    /// random padding via `fallbackCandidates`.
    static func generateBaseOptions(
        targetBase: String,
        sameKindBases: [String],
        otherBases: [String],
        fallbackCandidates: [String]
    ) -> [String] {
        var opts: [String] = [targetBase]
        var seen = Set<String>([targetBase])

        let pickFrom: ([String]) -> Void = { source in
            for candidate in source.shuffled() {
                if opts.count >= 4 { break }
        ...
        pickFrom(sameKindBases)
        if opts.count < 4 { pickFrom(otherBases) }
        if opts.count < 4 { pickFrom(fallbackCandidates) }

        return opts.shuffled()
    }

    /// Stage 2 distractors: 4 inflected (target) forms of the SAME base.
    /// Correct = `correctTarget`. `siblingTargets` ships from server; if it is
    /// empty or short, fall back to spelling-similar candidates so the grid is
    /// always full.
    static func generateSiblingTargetOptions(
        correctTarget: String,
        siblingTargets: [String],
        fallbackCandidates: [String]
    ) -> [String] {
        var opts: [String] = [correctTarget]
        var seen = Set<String>([correctTarget])

        for candidate in siblingTargets {
            if opts.count >= 4 { break }
            let trimmed = candidate.trimmingCharacters(in: .whitespaces)
            guard !trimmed.isEmpty, !seen.contains(trimmed) else { continue }
            opts.append(trimmed)
        ...
        // Fallback: spelling-similar from lesson catalog
        if opts.count < 4 {
            let scored = fallbackCandidates.map { c -> (String, Double) in
                let dist = levenshtein(c, correctTarget)
                let maxLen = Double(max(c.count, correctTarget.count))
                let sim = maxLen > 0 ? 1.0 - (dist / maxLen) : 0.0
                return (c, sim)
            }.sorted { $0.1 > $1.1 }
