...
struct GranularAnalyzer {
    ...
    static func processGranularMastery(
        ...
        // ✅ FIX: Use allBricks to scan the entire session database, not just the active group.
        let brickMatches = ContentAnalyzer.findRelevantBricks(
            in: target,
            meaning: meaning,
            bricks: engine.allBricks,
            targetLanguage: engine.lessonData?.target_language ?? "es"
        )
        let bricks = MasteryFilterService.resolveBricks(ids: Set(brickMatches), from: engine.allBricks)

        // 🔍 Diagnostic — every ripple announces itself: which pattern triggered
        //    it, what its solution looks like, and which bricks are about to be
        //    moved. Cross-pattern leaks (a ripple touching a brick that doesn't
        //    actually belong to the active pattern) become visually obvious.
        print("⚓ [GranularRipple] START | activePattern='\(engine.orchestrator?.activeState?.patternId ?? "—")' | type=\(type) | target='\(target)' | meaning='\(meaning)' | userInput='\(userInput)' | rippleBricks=\(bricks.map { $0.id })")
