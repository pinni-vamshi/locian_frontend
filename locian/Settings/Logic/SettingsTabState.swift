...
class SettingsTabState: ObservableObject {
    ...
    var defaultTargetLanguageName: String {
        defaultPair?.targetEnglishName.uppercased() ?? "UNKNOWN"
    }
    
    var activeLanguageNativeName: String {
        defaultPair?.targetNativeName.uppercased() ?? ""
    }
    ...
    func refreshPersonalization(completion: @escaping (Bool) -> Void) {
        ...
        let triggeredAt = ISO8601DateFormatter().string(from: Date())
        print("🔁 [SettingsTabState] refreshPersonalization triggered at \(triggeredAt) -> calling /api/user/intent/context")
    ...
    func handleRefresh(offset: CGFloat) {
        ...
        if offset > 110 {
            ...
            // Trigger both:
            // 1) personalization context refresh (/api/user/intent/context)
            // 2) language catalog + pairs refresh
            let group = DispatchGroup()

            group.enter()
            refreshPersonalization { _ in
                group.leave()
            }

            group.enter()
            GetAvailableLanguagesService.shared.fetch { [weak self] _ in
                self?.appState.loadAvailableLanguagePairs { _ in
                    group.leave()
                }
            }
